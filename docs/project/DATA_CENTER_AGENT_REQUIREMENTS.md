...
日期：2026-09-15。状态：用户方向与关键权限已确认；本文完成需求与技术方案，生产代码未实现、未验收。
...
|执行协作|Luna max 实现，主代理做需求、边界和代码审查；当前 Luna 回合遇额度限制，不宣称已开发|
...
唯一拟新增运行依赖：`sqlglot==30.18.0` 纯 Python 版本，用于 SQL AST、对象引用和限定语法检查；本轮只核验 PyPI 元数据（Python >=3.9、MIT、基础包无必需运行依赖），未安装。实现时加入锁定依赖及打包验证。它不是安全沙箱或数据库语义裁判，不能把“parse 成功”当成只读证明。
...
|阶段|Luna产出|主代理审查门槛|
|---|---|---|
|DC-00 基线|现有动作/驱动/导航/数据格式清单，脱敏测试夹具|没有遗漏六类能力；不复制真实配置|
|DC-01 Agent契约|类型、工具schema、模型适配、纯内存假模型/假数据库循环|两次工具调用→结果回传→基于证据回答；禁止工具零执行|
|DC-02 执行底座|SessionManager、只读policy、AST、预算/脱敏/取消、SQL各adapter|读写隔离、AST拒绝、原始数据类型、迟到结果测试|
|DC-03 只读闭环|接内网模型配置、六类只读工具、事件面板|沙箱库端到端；真实结果必须有query_id；不支持模型明确降级|
|DC-04 统一工作台|连接树、标签、结果模型、原生Prism原型与实现|SQL/Redis/Mongo三场景先评图，再核对实际窗口，非只查源码|
|DC-05 手动业务|事务、表格编辑、导入导出、历史收藏、SSH|提交/回滚/冲突/断线/恢复及hostkey测试|
|DC-06 迁移发布|兼容路由、旧数据迁移、打包资源、需求与开发回执|六类集成证据、回退说明、发布包启动与用户验收|
...
当前已完成：本地与DBX源码审查、用户范围确认、本需求与交接设计。当前未完成：实现、模型/数据库真实联调、UI原型新版本、测试和新发布包。Luna额度恢复前不以主代理静默代写生产代码替代用户指定分工。
