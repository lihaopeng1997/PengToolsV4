...
最终报告逐行列：22导航叶子、所有QDialog类、两种首页/侧栏renderer、加载8场景、浮窗3模式、calm单主题及旧主题配置兼容归一、窗口4档、行为对照与测试。每行附截图路径/用例命令/结果/未覆盖点。静态索引覆盖!=运行验证；green typecheck!=视觉通过；业务fixture通过!=实际服务集成通过。
