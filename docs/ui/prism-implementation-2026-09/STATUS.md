...
补齐此前缺少的FloatingShortcutsEditor与ImagePreviewDialog各13/16字号、模拟960×640屏幕的四张图，离屏运行均退出0；已查看两张16字号图。图片预览采用生成的DEMO图，不读用户图片；只对该类在诊断中模拟screen。现有类图集共有58张，包括三个长文本附加案例，仍未覆盖全部状态与临时对话框。

快捷入口编辑器实图暴露固定480项宽造成水平滚动、拖动符号缺字。现列表项保留48高度并自适应可用宽度，列表采用Prism圆角表面；新增currentColor/1.6描边的六点SVG拖动图标，替代文字字形，未改勾选、排序或保存算法。修正后的16字号图已查看，无水平滚动，全部勾选框可达。

Windows隔离 `tests.test_quick_panel_lifecycle` 17项与 `tests.test_prism_icons_brand` 17项通过，共34项；新增用例修复前复现横滚范围20，修复后检查中英文、520×480/440×360各行checkbox完整可见、窗口变化不改已选session。该检查不等于真实拖动或保存端到端验收。生成清单脚本只解析源码AST和诊断元数据，不导入业务模块、不读取用户记录。其余门槛按清单继续保留。

按用户补充提醒，新增[开发AI完整提示词](../../project/AI_DEVELOPMENT_PROMPT.md)，同步需求AI提示词、双AI开发规范和项目入口。交接覆盖基准核对、源码调用链、不变合同、最小测试、实际界面证据、提交/远端SHA、逐条验收和需求AI独立读代码返工闭环；将原型整目录与缺口清单列为必交材料。文档齐备不等于整套UI已完成。

## 2026-09-14 原生首页装饰透明与动画可见性

本批基于本地 `e4429a2747d3dc9755f0a8240333c4e1446ce3c0`。实际原生首页中PrismOrbWidget继承全局QWidget底色，形成120×120方块；新增专用对象名及透明背景规则，使环与玻璃中心直接叠在Hero渐变上。1440×900原生窗口重新运行退出0，初次样式修正图已查看并确认方块消失，动画修正后再次采样保留最新源码记录，静态外观不变。

同时复现隐藏父窗口中的Orb构造后仍运行，以及已显示时关闭动效仍运行两个问题。现在初始化仅在可见且允许动效时启动；绘制值回调发现不可见或减弱动效时暂停，不继续改变相位。保持原4.8s周期、浮动/旋转/缩放范围、单动画实例及原隐藏/重显恢复行为。未改任务状态或页面业务逻辑。

Windows隔离 `tests.test_prism_dashboard_motion` 2项新增运行回归修复前均失败、修复后通过：构造隐藏、两相位实际图像变化、隐藏时进度不变、重显复用、减弱动效画面与几何不变。`tests.test_prism_dashboard` 原8项结构检查通过；结构检查不替代运行视觉或CPU性能证据。其他尺寸的原生截图保留上一批状态，完整UI与性能验收仍未完成。
...
通过现有隔离运行入口进行一次临时Qt状态探针：两个实际QToolButton应用生产样式，分别赋予焦点、按下、禁用并抓取图像，确认FOCUS_RING、TABLE_SELECT、DISABLED_BG颜色确实出现；焦点前后sizeHint和geometry相同，原菜单对象与动作仍在。探针退出0，未保存为回归用例或截图，不代表完整菜单交互或读屏软件验收。另运行 `python scripts/diagnostics/prism_web_runtime.py --test-module tests.test_agent_workbench_layout`，Windows隔离10项通过。

上一批提交尚未推送成功，远程核对仍为3082188；网络错误不等于远程已更新。本记录中的源码基准是本地提交，交付时需再次报告远程实际状态。完整UI验收继续保留未完成。

## 2026-09-14 需求、Agent与设置宽窗口检查

本批基于 `3082188fb31aa28b38a12b9ec59b876708ce90cc`，补充需求管理、Agent工作台、设置三个1440×900实际集成窗口截图与记录，侧栏展开宽248、DPR1.5，窗口尺寸、网页加载与bridge就绪、菜单选中检查通过。隔离环境使用临时配置，未连接真实服务。图集现有67张记录，并非67项最终验收。

Agent附件菜单按钮原为原生方框，新增专用对象名并复用现有圆角工具按钮样式。初次对象名被apply_button覆盖，现置于该调用之后；重新运行Agent窗口退出0，查看最新截图确认圆角边框生效。菜单及原操作不变。最新Agent记录注明两份未提交UI源码路径，其他两图保留各自采样时的记录。

Windows隔离 `python scripts/diagnostics/prism_web_runtime.py --test-module tests.test_agent_workbench_layout` 10项通过；Agent实际窗口复验退出0，图集生成成功，差异检查通过。本批仅覆盖三个入口的初始状态；浏览器图集交互、完整尺寸与状态矩阵、性能及人工视觉验收仍待完成，NoSQL线程调整继续保留待确认。
