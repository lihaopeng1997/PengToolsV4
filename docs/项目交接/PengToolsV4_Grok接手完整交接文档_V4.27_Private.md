...
5. 只改本轮范围；定向测试；`.\build_private_release.ps1`；`git commit` + `git push origin main`。
...
```
PengToolsV4/
├── run.py                 # 入口：DPI、主题、单实例、主窗口
├── main_window.py         # 侧栏导航、Stack、托盘/关闭、模块联动、关于/彩蛋
├── config.py              # APP_*、local_data_dir、各类 JSON 路径与默认值
├── AGENTS.md              # 给 AI/开发者的硬规则（优先读）
├── requirements.txt
├── build_private_release.ps1   # ★ 日常私人包构建
├── build_release.ps1           # 标准包（慎用）
├── panels/                # 各导航模块 UI（PyQt）
├── tools/                 # 无界面业务逻辑
├── ui/                    # 主题、图标、响应式、弹窗、托盘、悬浮栏
├── resources/             # style.qss、图标、build_info、发版 Excel 模板、学习种子
├── data/                  # 开发态用户数据（gitignore，勿提交）
├── tests/                 # 定向单元/面板烟雾测试
├── PrivateInstaller/      # 私人安装模板
├── docs/
│   ├── 项目交接/          # 本文与历史交接
│   ├── 架构/
│   └── ui/                # 历史需求/蓝图（实现以代码为准）
...
4. 定向测试（见第 8 节）。
5. 若可交付：提交并推送（见 7.3）。
6. 私人包：`.\build_private_release.ps1`（若 EXE 被占用先结束 `PengToolsHub_Private` 进程）。
